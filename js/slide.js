$(document).ready(function() {
  var slider = $('.slider'),
    ...
    slideCount = $('.slider .kar').length,               // Количество слайдов
    l = $('.l'),                      // Кнопка "назад"
    r = $('.r'),                      // Кнопка "вперед"
  ...
  $('div.kar:last').clone().prependTo('.slider');   // Копия последнего слайда помещается в начало.
  $('div.kar').eq(1).clone().appendTo('.slider');   // Копия первого слайда помещается в конец.
  ...
  l.click(function() {                               // Нажата кнопка "назад"
  ...
  r.click(function() {                               // Нажата кнопка "назад"
    if (slider.is(':animated')) { return false; }       // Если не происходит анимация
    var course2 = course;                               // Временная переменная для хранения значения course
    course = 1;                                         // Устанавливается направление слайдера справа налево
    animate();                                          // Вызов функции animate()
  ...
  slider.add(r).add(l).hover(function() {         // Если курсор мыши в пределах слайдера
